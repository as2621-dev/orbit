...
Used by youtube_yt.py (yt-dlp subscription/transcript fetches) and bird_x.py
(the vendored Node.js X client). Both need the same os.setsid/killpg cleanup
dance on timeout to avoid orphaning child processes (a hung yt-dlp or Node
process must not leak past Orbit's run).

Lifted near-verbatim from the last30days reference so later Orbit modules drop
in unchanged.
"""
